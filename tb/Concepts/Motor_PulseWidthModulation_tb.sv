...
module Motor_PulseWidthModulation_tb();
	reg r_Clock;
	...
	localparam c_PWM_FREQUENCY	= 20_000;
	localparam c_PERIOD_TICKS	= c_CLOCK_FREQUENCY / c_PWM_FREQUENCY;
	...
	// DUT
	// =========================================================
	Motor_PulseWidthModulation #(
		.g_CLOCK_FREQUENCY(c_CLOCK_FREQUENCY),
		.g_PWM_FREQUENCY(c_PWM_FREQUENCY)
	) dut (
		.i_Clock(r_Clock),
	...
	initial begin
		...
		for (i = 0; i < c_NUMBER_OF_TESTS; i++) begin
			...
			for (j = 0; j < c_PERIOD_TICKS; j++) begin
				t_Step_Clock();
				if (w_PWM_Signal)
					highCount++;
			end
			...
			expected = (r_Duty_Cycle * c_PERIOD_TICKS) / c_RESOLUTION;
